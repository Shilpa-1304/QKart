...
const addProductToCart = async (user, productId, quantity) => {
  ...
  if(!cart){
    ...
    try{
      ...
      throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR,"cart creation fails")
    }
  }
 if(cart==null){
 ...
 let newCart=cart.cartItems.filter((item)=>{
  ...
  if(newCart.length>0){
    throw new ApiError(httpStatus.BAD_REQUEST,"Product already in cart.Use sidebar to update or remove product from cart.")
  }
  else{
...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  if(userCart==null){
    throw new ApiError(httpStatus.BAD_REQUEST,"User doesn't have a cart. Use POST to create cart and add a product")
  }
...
const deleteProductFromCart = async (user, productId) => {
  let cart=await Cart.findOne({email:user.email});
  if(!cart){
    throw new ApiError(httpStatus.BAD_REQUEST,"User doesn't have a cart.")
  }
